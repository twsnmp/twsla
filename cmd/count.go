...
func init() {
	...
	countCmd.Flags().StringVarP(&extract, "extract", "e", "", "Extract key")
	countCmd.Flags().StringVar(&delimiter, "delimiter", ":", "Delimiter")
}
...
func (m countModel) headerView() string {
	title := titleStyle.Render(fmt.Sprintf("Results %d/%d %d %v %s", m.msg.Hit, m.msg.Lines, len(countList), m.msg.Dur, m.lastSort))
